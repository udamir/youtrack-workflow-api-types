...
export type IssueSingleFieldType<T extends string> =
  | string
  | number
  | User
  | State<T>
...
interface IssueLinkPrototypeRequirement extends BaseRequirement<typeof IssueLinkPrototype> {
  /**
 ...
 */
export type FieldNameFromRequirement<K extends string, R extends Requirement> = R["name"] extends string ? R["name"] : K

/**
 * Transforms a requirement into its corresponding entity type
 */
export type ProjectCustomFieldFromRequirement<R extends Requirement> = R["type"] extends FieldType.stringType
  ? ProjectCustomField
  : R["type"] extends FieldType.textType
    ? TextProjectCustomField
    : R["type"] extends FieldType.periodType
      ? PeriodProjectCustomField
      : R["type"] extends FieldType.integerType
        ? ProjectCustomField
        : R["type"] extends FieldType.floatType
          ? ProjectCustomField
          : R["type"] extends FieldType.dateType
            ? PeriodProjectCustomField
            : R["type"] extends FieldType.dateTimeType
              ? PeriodProjectCustomField
              : R["type"] extends FieldType.enumFieldType
                ? BundleProjectCustomField
                : R["type"] extends typeof EnumField
                  ? BundleProjectCustomField
                  : R["type"] extends FieldType.userFieldType
                    ? UserProjectCustomField
                    : R["type"] extends typeof OwnedField
                      ? UserProjectCustomField
                      : R["type"] extends typeof State
                        ? BundleProjectCustomField
                        : R["type"] extends typeof ProjectVersion
                          ? BundleProjectCustomField
                          : R["type"] extends typeof Build
                            ? BundleProjectCustomField
                            : R["type"] extends typeof UserGroup
                              ? GroupProjectCustomField
                              : R["type"] extends typeof Project
                                ? Project
                                : R["type"] extends typeof Issue
                                  ? Issue
                                  : R["type"] extends typeof User
                                    ? User
                                    : R["type"] extends typeof Tag
                                      ? Tag
                                      : never
/**
 ...
 */
export type ProjectCustomFieldsFromRequirements<R extends Record<string, Requirement>> = {
  [K in keyof R as FieldNameFromRequirement<K & string, R[K]>]: ProjectCustomFieldFromRequirement<R[K]>
}
 ...
 */
export type FieldFromRequirement<T extends ActionUserInputType> = T extends FieldType.dateTimeType | FieldType.dateType
  ? Date
  : T extends FieldType.integerType | FieldType.floatType | FieldType.periodType
    ? number
    : T extends FieldType.stringType
      ? string
      : T extends EnumField
        ? string
        : T extends Build
          ...
          : T extends Issue
            ...
            : T extends Tag
              ? Tag
              : T extends OwnedField
                ? OwnedField
                : T extends Project
                  ? Project
                  : T extends ProjectVersion
                    ? ProjectVersion
                    : T extends User
                      ? User
                      : T extends UserGroup
 ...
 */
export type SLABreachContext<R extends Requirements, F extends IssueFields, W extends string> = RuleContext<R, F, W> & {
 ...
 * @param ctx The execution context. Along with the parameters listed below, the context also contains objects that you describe as Requirements.
 */
export type slaBreachFunction = <R extends Requirements, F extends IssueFields, W extends string>(
  ctx: SLABreachContext<R, F, W>,
) => void
 ...
 * @param ctx The execution context. Along with the parameters listed below, the context also contains objects that you describe as Requirements.
 */
export type slaEnterFunction = <R extends Requirements, F extends IssueFields, W extends string>(
  ctx: RuleContext<R, F, W>,
) => void
 ...
 * @param ctx The execution context. Along with the parameters listed below, the context also contains objects that you describe as Requirements.
 */
export type slaGuardFunction = <R extends Requirements, F extends IssueFields, W extends string>(
  ctx: RuleContext<R, F, W>,
) => boolean
